...
CLIENT_ID = 'eebb54b9c420456899668bd0a7790d0d'
CLIENT_SECRET = '9d302e2bfb3143e99b47eebf473621c7'

auth_manager = SpotifyClientCredentials(client_id=CLIENT_ID, client_secret=CLIENT_SECRET)
sp = spotipy.Spotify(auth_manager=auth_manager)

original_title = '<p style="font-family:Roboto; font-size: 45px;"><font color = "#4E6E81">Song Recomendation System</font></p>'
st.markdown(original_title, unsafe_allow_html=True)
#st.header("Spotify Web application")

search_choice = ["Song","Artist","Album","Genre"]
search_selected = st.sidebar.selectbox("Please select your choice",search_choice)
...
if search_keyword is not None and len(str(search_keyword)) > 0:
    if search_selected == 'Song':
        original_title = '<p style="font-family:Roboto; color:Gray; font-size: 20px;"><b>Starting Song Search<b></p>'
        st.markdown(original_title, unsafe_allow_html=True)
        ...
        original_title = '<p style="font-family:Roboto; color:Gray; font-size: 20px;"><b>Starting Artist Search<b></p>'
        st.markdown(original_title, unsafe_allow_html=True)
    ...
    if search_selected == 'Album':
        original_title = '<p style="font-family:Roboto; color:Gray; font-size: 20px;"><b>Starting Album Search<b></p>'
        st.markdown(original_title, unsafe_allow_html=True)
...
if selected_track is not None and len(tracks) > 0:
    ...
    if track_id is not None:
        ...
        if selected_track_choice == 'Song Features':
            ...
            ##############################
            #extract the genre of the song from the track id
            ##############################
            artist_code = tracks['tracks']['items'][0]['album']['artists'][0]['uri']
            #st.write("artist_code: ", artist_code)
            recommended_genre = sp.artist(artist_code)['genres'][0]
            #st.write("recommended_genre: ", recommended_genre)
            

            track_data = sp.search(q='genre:'+recommended_genre,type='track',limit=20)
            track_result = []
            ...
            final_df = final_df[['Song Name','acousticness', 'danceability', 'energy', 'instrumentalness', 'liveness', 'speechiness', 'valence']]
            ...
            st.write(neigh.predict([[0.641,0.839,0.473,0.0005,0.0939,0.0607,0.0644]]))
                

    else:
        st.write("No track found")
